...
x_label=r'1/N'
x_sec_label=r'N'
y_label=r'$\Delta E_\mathrm{HL}$ (eV)'
...
def gap(df):
    return df["lumo"]-df["homo"]

#Data processing
frames = [df0,df1,df2,df3]
for frame in frames:
    frame["gap"] = frame.apply(gap, axis=1)
...
ax.scatter(1/df0["natoms"],df0["gap"],marker="o",label=r'$I_h$',
        color=palette[0],s=ms,linewidths=lw)
ax.scatter(1/df1_1["natoms"],df1_1["gap"],marker="o",label=r'C$_{60+30n}$',
        facecolor='none',edgecolor=palette[1],s=ms,linewidths=lw)
ax.scatter(1/df1_2["natoms"],df1_2["gap"],marker="s",label=r'C$_{60+30n+10}$',
        facecolor='none',edgecolor=palette[2],s=ms,linewidths=lw)
ax.scatter(1/df1_3["natoms"],df1_3["gap"],marker="v",label=r'C$_{60+30n+20}$',
        facecolor='none',edgecolor=palette[3],s=ms,linewidths=lw)
ax.scatter(1/df2["natoms"],df2["gap"],marker="x",label=r'C$_{60+12n}$',
        color=palette[4],s=ms,linewidths=lw)
ax.scatter(1/df3["natoms"],df3["gap"],marker="+",label=r'C$_{60+18n}$',
...
if show_annot:
    ...
    for xi, yi, text in zip(df0["natoms"], df0["gap"], annotations):
        if xi == 60:
            ax.annotate(text,
