...
def mu(df):
    return -1*(df["ie"]+df["ea"])/2

def hardness(df):
    return df["ie"]-df["ea"]

def e_index(df):
    return df["mu"]**2/(2*df["hardness"])

# Data processing
frames = [df0,df1,df2,df3]
for frame in frames:
    frame["ie"] = frame.apply(ie, axis=1)
    frame["ea"] = frame.apply(ea, axis=1)
    frame["mu"] = frame.apply(mu, axis=1)
    frame["hardness"] = frame.apply(hardness, axis=1)
    frame["e_index"] = frame.apply(e_index, axis=1)
...
ax.scatter(df0["natoms"],df0["e_index"],marker="o",label=r'$I_h$',
        color=palette[0],s=ms,linewidths=lw)
ax.scatter(df1_1["natoms"],df1_1["e_index"],marker="o",label=r'C$_{60+30n}$',
        facecolor='none',edgecolor=palette[1],s=ms,linewidths=lw)
ax.scatter(df1_2["natoms"],df1_2["e_index"],marker="s",label=r'C$_{60+30n+10}$',
        facecolor='none',edgecolor=palette[2],s=ms,linewidths=lw)
ax.scatter(df1_3["natoms"],df1_3["e_index"],marker="v",label=r'C$_{60+30n+20}$',
        facecolor='none',edgecolor=palette[3],s=ms,linewidths=lw)
ax.scatter(df2["natoms"],df2["e_index"],marker="x",label=r'C$_{60+12n}$',
        color=palette[4],s=ms,linewidths=lw)
ax.scatter(df3["natoms"],df3["e_index"],marker="+",label=r'C$_{60+18n}$',
...
if show_annot:
    ...
    for xi, yi, text in zip(df0["natoms"], df0["e_index"], annotations):
        if xi == 60:
            ax.annotate(text,
