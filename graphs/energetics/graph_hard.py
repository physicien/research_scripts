...
ax.scatter(df0["natoms"],df0["hardness"],marker="o",label=r'$I_h$',
        color=palette[0],s=ms,linewidths=lw)
ax.scatter(df1_1["natoms"],df1_1["hardness"],marker="o",label=r'C$_{60+30n}$',
        facecolor='none',edgecolor=palette[1],s=ms,linewidths=lw)
ax.scatter(df1_2["natoms"],df1_2["hardness"],marker="s",
           label=r'C$_{60+30n+10}$',facecolor='none',edgecolor=palette[2],
           s=ms,linewidths=lw)
ax.scatter(df1_3["natoms"],df1_3["hardness"],marker="v",
           label=r'C$_{60+30n+20}$',facecolor='none',edgecolor=palette[3],
           s=ms,linewidths=lw)
ax.scatter(df2["natoms"],df2["hardness"],marker="x",label=r'C$_{60+12n}$',
        color=palette[4],s=ms,linewidths=lw)
ax.scatter(df3["natoms"],df3["hardness"],marker="+",label=r'C$_{60+18n}$',
...
if show_annot:
    ...
    for xi, yi, text in zip(df0["natoms"], df0["hardness"], annotations):
        if xi == 60:
            ax.annotate(text,
