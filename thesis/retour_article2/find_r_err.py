...
def r_iso(df):
    ...
    iso=iso_pol(df)
    logK=tlog(df)
    result=abs(3/2*ie_pyr*ie/(ie_pyr+ie)*conv_eV/R/T*ipyr*iso/(logK-logF))**(1/6)
...
def r_eff(df):
    ...
    eff=iso - ani/6
    logK=tlog(df)
    result=abs(3/2*ie_pyr*ie/(ie_pyr+ie)*conv_eV/R/T*ipyr*eff/(logK-logF))**(1/6)
    return '{:.3f}'.format(result)
...
def r_full(df):
    ...
    logK=tlog(df)
    result=abs(3/2*ie_pyr*ie/(ie_pyr+ie)*conv_eV/R/T*tot/(logK-logF))**(1/6)
    return '{:.3f}'.format(result)
...
df["iso_r"] = df.apply(r_iso, axis=1)
df["eff_r"] = df.apply(r_eff, axis=1)
df["full_r"] = df.apply(r_full, axis=1)

print(df[["name","iso_r","eff_r","full_r"]])
#print(df[["name","iso","iso_r"]])
#print(df[["name","eff","eff_r"]])
#print(df[["name","c_factor","full_r"]])
